...
$check_sql = "SELECT * FROM favorites WHERE user_id = ? AND recipe_id = ?";
...
if ($result->num_rows > 0) {
    ...
    // Update recipe owner points (-2 points)
...
// Get the updated count of favorites (total)
$count_sql = "SELECT COUNT(*) as count FROM favorites WHERE recipe_id = ?";
$stmt = $conn->prepare($count_sql);
$stmt->bind_param("i", $recipe_id);
$stmt->execute();
$count_result = $stmt->get_result();
$count_row = $count_result->fetch_assoc();
$new_favorite_count = $count_row['count'];

// Get count EXCLUDING recipe owner's favorite
$count_excluding_owner_sql = "SELECT COUNT(*) as count FROM favorites WHERE recipe_id = ? AND user_id != ?";
...
// Return the status and the updated counts
echo json_encode([
    'status' => $status, 
    'count' => $new_favorite_count,
